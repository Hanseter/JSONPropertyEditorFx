...
class IdReferenceControl(private val schema: EffectiveSchema<StringSchema>, private val context: EditorContext) : ControlWithProperty<String?> {
    ...
    init {
        ...
        TextFields.bindAutoCompletion(control) { request ->
            val proposals = context.refProvider.calcCompletionProposals(request.userText, context.editorObjId, schema.baseSchema)
            if (regex != null) {
    ...
    private fun idChanged(id: String?) {
        ...
        val desc = context.refProvider.getReferenceDescription(id, context.editorObjId, schema.baseSchema)
